...
![PDF p.166: Pipelined KNU Sandwich Task Order Diagram vs Sequential Processing](img/Pasted%20image%2020251201161334.png)(ㅋㅋㅋ 유쾌하시네)
...
![PDF p.185: Pipelined RISC-V Datapath showing concurrent execution of 5 instructions](img/Pasted%20image%2020251201161501.png)

5개의 단계는 다음과 같다.
1. **IF (Instruction Fetch)**: 메모리에서 명령어를 가져온다.
2. **ID (Instruction Decode)**: 명령어를 해석하고 레지스터 값을 읽는다.
3. **EX (Execute)**: ALU 연산을 수행하거나 주소를 계산한다.
4. **MEM (Memory Access)**: 데이터 메모리에 접근한다(Load/Store).
5. **WB (Write Back)**: 결과를 레지스터에 기록한다.

Single-Cycle에서는 $t_{instruction} = 800ps$라면 클럭 주기($t_{cycle}$)도 800ps여야 하지만, Pipelined 방식에서는 가장 느린 단계의 지연 시간(예: 200ps)에 맞춰 클럭 주기를 200ps로 줄일 수 있다. 결과적으로 처리량은 최대 4배까지 증가할 수 있다.
...
각 단계 사이에는 파이프라인 레지스터(Pipeline Registers)가 존재하여 이전 단계의 결과와 제어 신호(Control Signals)를 다음 단계로 전달한다. 이는 각 단계가 독립적으로 동작하도록 하며, 동시에 서로 다른 명령어의 데이터를 처리할 수 있게 한다.
	...
	![PDF p.206: Detailed Diagram of Pipelined Datapath with Control Logic and Pipeline Registers](img/Pasted%20image%2020251201161535.png)
...
Pipelining을 통해 이상적으로는 $Speedup = Pipeline Depth$를 달성할 수 있다. 5단계 파이프라인의 경우 최대 5배의 속도 향상이 가능하지만, 실제로는 다음과 같은 이유로 그보다 낮다.
...
Pipeline CPI는 다음과 같이 계산된다.
$CPI = Ideal CPI + Structural Stalls + Data Hazard Stalls + Control Stalls$
이상적인 CPI는 1이지만, 각종 Hazard로 인한 Stall 때문에 실제 CPI는 1보다 커지게 된다.
...
1. **Structural Hazard**: 하드웨어 자원이 부족하여 여러 명령어가 동시에 같은 자원에 접근하려 할 때 발생한다. (Resource Conflict)
2. **Data Hazard**: 명령어 간의 데이터 의존성(Data Dependency)으로 인해, 이전 명령어의 데이터 읽기/쓰기가 완료될 때까지 기다려야 하는 상황이다.
3. **Control Hazard**: 분기(Branch) 명령어 등으로 인해 실행 흐름이 변경되어, 다음에 실행할 명령어가 결정되지 않은 상태에서 발생한다.
...
![PDF p.219: Pipeline Diagram illustrating separate Instruction and Data Memory to avoid Structural Hazards(img/Pasted%20image%2020251201161712.png)

해결책은 다음과 같다.
* **Instruction Memory와 Data Memory 분리**: 하버드 아키텍처(Harvard Architecture)와 같이 명령어 메모리(IMEM)와 데이터 메모리(DMEM)를 물리적으로 분리하거나, 별도의 캐시(I-Cache, D-Cache)를 사용하여 동시에 접근 가능하게 한다.
* **Register File 포트 확장**: 레지스터 파일이 동시에 읽기(2개)와 쓰기(1개)를 지원하도록 포트를 구성하여, Decode(ID) 단계의 읽기와 Write-Back(WB) 단계의 쓰기가 동시에 일어날 수 있게 한다.

### Data Hazards: Types and Solutions

Data Hazard는 데이터 의존성으로 인해 발생하며, 주로 세 가지 케이스가 있다.

**1. Register Access Hazard (Read/Write Conflict)**
동일한 사이클에 한 명령어는 WB 단계에서 레지스터에 쓰고, 다른 명령어는 ID 단계에서 같은 레지스터를 읽으려 할 때 발생한다.
* **해결책**: 레지스터 파일을 클럭의 전반부(First half)에 쓰고, 후반부(Second half)에 읽도록 설계하여, 한 사이클 내에서 쓰기 후 읽기(Write-then-Read)가 가능하게 한다.

**2. ALU Result Hazard (RAW Hazard)**
이전 명령어의 연산 결과(EX 단계)가 아직 레지스터에 기록(WB 단계)되지 않았는데, 다음 명령어가 그 값을 필요로 할 때 발생한다.

![PDF p.231: Data Hazard causing wrong value read without forwarding](img/Pasted%20image%2020251201161740.png)

* **Solution 1: Stalling (Interlock)**
하드웨어적으로 Hazard를 감지하여 파이프라인을 멈춘다(Stall). 제어 신호를 조작하여 PC와 IF/ID 레지스터 업데이트를 막고, ID/EX 단계에 NOP(No Operation)을 삽입한다. 이는 소프트웨어 호환성을 유지하지만 성능 저하를 일으킨다.

![PDF p.240: Hardware Interlock inserting NOP bubbles to resolve Data Hazard](img/Pasted%20image%2020251201161811.png)
...
![PDF p.254: Full Bypass Logic Diagram showing multiplexers selecting forwarded data](img/Pasted%20image%2020251201161850.png)

**3. Load Data Hazard (Load-Use Hazard)**
Load 명령어는 MEM 단계가 끝나야 데이터가 준비되므로, 바로 다음 명령어가 그 데이터를 필요로 하면 Forwarding만으로는 해결할 수 없다. (이 부분은 다음 강의 주제인 Data Hazard III에서 다룸)

### Pipeline Control and Stall Logic

Stall과 Bypass는 상호보완적이다. Bypass가 가능한 경우에는 Bypass를 통해 성능 저하 없이 Hazard를 해결하고, Bypass가 불가능한 경우(예: Load-Use Hazard)에는 Stall을 사용하여 파이프라인을 멈춰야 한다.
Stall Logic은 Hazard Detection Unit에 의해 제어되며, 파이프라인 레지스터의 쓰기 신호(WE)와 제어 신호 멀티플렉서를 조작하여 NOP을 삽입한다.
...
![PDF p.259: Diagram showing unresolved data hazard with load followed by dependent add instruction](img/Pasted%20image%2020251201162915.png)
예를 들어, `lw x3, 8(x2)` 다음에 `add x4, x2, x3`가 오는 경우, `lw`는 MEM 단계가 끝나야 `x3` 값을 사용할 수 있지만, `add`는 EX 단계에서 `x3` 값을 필요로 한다. Forwarding은 시간을 거슬러 데이터를 보낼 수 없으므로, 이 경우에는 반드시 Stall을 사용하여 파이프라인을 멈춰야 한다.
...
1. **Hardware Interlock (Stall)**: 하드웨어가 Hazard를 감지하고 파이프라인을 1사이클 멈춘다(Stall).
    ...
    * 일치할 경우, PC와 IF/ID 파이프라인 레지스터의 업데이트를 막고(Disable Write), ID/EX 레지스터에 NOP(No Operation)을 삽입하여 뒷 명령어의 진행을 1사이클 늦춘다.
    ...
    ![PDF p.265: Diagram illustrating stalling on load-to-use dependency by inserting a NOP](img/Pasted%20image%2020251201162941.png)

2. **Code Scheduling (Software Solution)**: 컴파일러가 명령어 순서를 재배치하여 Hazard를 피한다.
    * Load 명령어와 이를 사용하는 명령어 사이에 의존성이 없는 다른 명령어를 삽입하여 거리를 둔다.
    * NOP을 삽입하는 것보다 유용한 명령어를 채워 넣는 것이 성능상 유리하다.

    ![PDF p.263: Code scheduling example reordering instructions to avoid load-use hazard](img/Pasted%20image%2020251201163005.png)
...
![PDF p.279: Control Hazard showing incorrect instructions entering pipeline after a branch](img/Pasted%20image%2020251201163035.png)
...
1.  **Structural Hazards**: 하드웨어 자원 충돌.
2.  **Data Hazards**: 데이터 의존성 문제.
    * 해결책: Stalling, Forwarding, Scheduling.
3.  **Control Hazards**: 분기 명령어로 인한 흐름 제어 문제.
...
![PDF p.304: Superscalar Processor Architecture Overview showing multiple execution units](img/Pasted%20image%2020251201163130.png)
...
$CPI = \frac{Time}{Program} \div (\frac{Instructions}{Program} \times \frac{Time}{Cycle})$
...
![PDF p.338: Detailed Diagram of In-Order Dual-Issue TinyRV1 Processor](img/Pasted%20image%2020251201163156.png)
...
1.  **Fetch**: 한 번에 여러 명령어를 Fetch해야 한다. 캐시 블록 경계에 걸쳐 있는 경우(Unaligned Fetch) 복잡해진다.
    * 해결책: Instruction Queue, Loop Stream Detector 등을 사용한다.
2.  **Decode**: 여러 개의 Decoder가 필요하다.
3.  **Issue**: 명령어 간의 의존성을 확인하고 동시에 실행 가능한지 판단해야 한다. (복잡도는 $N^2$에 비례)
4.  **Register Read**: 포트 수가 늘어나면 레지스터 파일의 크기와 접근 시간이 증가한다.
5.  **Bypass**: $N^2$개의 Bypass 경로가 필요하여 배선이 복잡해지고 지연 시간이 증가한다.
...
Instruction-Level Parallelism (ILP)은 명령어들의 실행을 중첩시켜 성능을 향상시키는 기법이다. 앞서 배운 Scalar Pipeline (부분 중첩)이나 Superscalar Pipeline (전체 중첩)이 이에 해당한다.
ILP를 극대화하기 위해서는 명령어 스케줄링이 필수적이다.
...
1. **True Dependence (Data Dependence)**: 명령어 2가 명령어 1의 결과를 필요로 하는 경우 (Read-After-Write). 이는 데이터 흐름을 나타내는 진짜 의존성이다.
2. **Anti-Dependence**: 명령어 2가 쓸 레지스터를 명령어 1이 아직 읽고 있는 경우 (Write-After-Read). 레지스터 이름을 바꾸면(Renaming) 해결되는 가짜 의존성(False Dependence)이다.
3. **Output Dependence**: 명령어 1과 2가 같은 레지스터에 쓰려고 하는 경우 (Write-After-Write). 이 또한 Renaming으로 해결 가능한 가짜 의존성이다.
...
In-Order Pipeline은 명령어를 프로그램 순서대로 인출(Fetch), 디코드(Decode), 실행(Execute)한다.
Cache Miss나 긴 지연 시간의 연산(Floating Point 등)이 발생하면 파이프라인 전체가 멈추는(Stall) 문제가 있다. 특히 Execute 단계가 하나뿐이거나 Blocking 방식이라면 Structural Hazard가 발생하여 성능 저하가 심각해진다.
...
1. **Stall-on-Miss (Simple Issue Policy)**: Cache Miss와 같은 긴 지연 이벤트가 발생하면 파이프라인 전체를 즉시 멈춘다. 구현이 간단하고 전력 소모가 적지만 성능이 낮다. (예: MIPS 5-stage pipeline)
2. **Stall-on-Use (Aggressive Issue Policy)**: 데이터가 실제로 필요할 때(Use)까지 Stall을 늦춘다. Register Read 단계에서 RAW 해저드를 감지하고 Stall을 건다. 이를 위해서는 Scoreboard와 같은 추가적인 하드웨어와 복잡한 제어 로직이 필요하다.
...
이 문제를 근본적으로 해결하기 위해서는 **Out-of-Order Execution**이 필요하다.
...
1.  **In-Order Front-End**: Fetch -> Decode -> Rename -> Dispatch. 명령어를 순서대로 가져와 디코딩하고 리네이밍한 후 Issue Queue로 보낸다.
2.  **Out-of-Order Execution Core**: Issue -> Register Read -> Execute -> Writeback. 피연산자가 준비된 명령어부터 순서에 상관없이 실행하고 결과를 쓴다.
3.  **In-Order Retirement (Commit)**: 실행이 완료된 명령어들을 프로그램 순서대로 확정(Commit)한다. 이는 예외 처리나 인터럽트 발생 시 정확한 상태(Precise State)를 보장하기 위함이다.
...
![PDF p.130: Issue Queue Operation showing instruction readiness check based on physical registers](img/Pasted%20image%2020251201163710.png)
...
![PDF p.134: OOO Execution Diagram showing Select and Wakeup mechanism](img/Pasted%20image%2020251201163743.png)
...
**Recovery (복구)**
분기 예측이 틀렸을 때(Misprediction), 잘못된 경로의 명령어들을 취소하고 프로세서 상태를 복구해야 한다.
...
**Commit (은퇴)**
명령어 실행이 완전히 끝나고, 더 이상 취소될 가능성이 없을 때 결과를 영구적으로 반영한다.
...
![PDF p.149: Register Renaming Commit Example illustrating freeing of overwritten physical registers](img/Pasted%20image%2020251201163807.png)
...
![PDF p.206: Example of bad interleaving where Load speculatively executes but gets wrong value](img/Pasted%20image%2020251201164130.png)
...
![PDF p.211: AMD Zen Microarchitecture Block Diagram illustrating complex OoO pipeline stages](img/Pasted%20image%2020251201164156.png)

### Summary: Advanced Scheduling Techniques

프로세서 성능을 높이기 위한 스케줄링 기법은 크게 두 가지로 요약된다.

1. **Code Scheduling (Static)**: 컴파일러가 수행한다. Pipeline stall을 줄이고 ILP를 높이기 위해 명령어 순서를 바꾼다. 하지만 분기, 레지스터 부족, 메모리 에일리어싱 등의 한계가 있다.
2. **Dynamic Scheduling (Hardware)**: 하드웨어가 런타임에 수행한다. Register Renaming으로 가짜 의존성을 제거하고, Issue Queue와 ROB를 이용해 명령어들을 Out-of-Order로 실행하며, Load/Store Queue로 메모리 접근을 최적화한다. 하드웨어 복잡도는 높지만 런타임 정보를 활용하여 최적의 성능을 이끌어낸다.
...
* **조건**:
    1. 예외를 발생시킨 명령어 이전의 모든 명령어는 완료되어야 한다.
    2. 예외를 발생시킨 명령어와 그 이후의 명령어들은 완료되지 않아야 하며(아무런 상태 변경도 없어야 함), 예외 처리 후 재실행 가능해야 한다.
