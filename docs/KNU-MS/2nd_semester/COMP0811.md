...
#### 1.1. ISA의 정의
[cite_start]ISA (Instruction Set Architecture)는 하드웨어와 소프트웨어 사이의 인터페이스를 정의하는 계약이다. [cite: 133, 156]
![PDF p.9: ISA as the interface between software and hardware]
* [cite_start]ISA는 소프트웨어(운영체제, 애플리케이션)가 하드웨어(CPU, Mem, I/O)를 제어하기 위한 기능적 명세를 정의한다. [cite: 158]
* [cite_start]ISA는 하드웨어가 이해하는 비트 패턴(명령어)과 [cite: 161, 166] [cite_start]레지스터, 메모리와 같은 저장 위치 및 접근 방법을 정의한다. [cite: 158, 159]
* [cite_start]명령어의 구현 방식, 속도, 전력 소모 등 비기능적 측면은 ISA 계약에 포함되지 않는다. [cite: 162-164]

#### 1.2. ISA vs. Microarchitecture
* **ISA (Architecture):** 소프트웨어에 보이는 기능적 명세이다. (예: 컴퓨터는 덧셈을 수행할 수 있다)[cite_start]. [cite: 110, 112, 172, 173]
* [cite_start]**Microarchitecture:** ISA를 실제로 구현하는 하드웨어 설계이다 (예: 특정 데이터패스와 제어 로직을 가진 설계). [cite: 113, 116, 174, 175, 186] [cite_start]이는 소프트웨어에게는 보이지 않는다. [cite: 175]

#### 1.3. 실행 모델 (von Neumann)
![PDF p.16: The von Neumann Architecture diagram]
[cite_start]현대 컴퓨터는 대부분 von Neumann 모델을 기반으로 한다. [cite: 263]
* [cite_start]컴퓨터는 CPU(중앙 처리 장치), Main Memory(메인 메모리), Input/Output(입출력 장치)로 구성된다. [cite: 273-278]
* [cite_start]CPU는 다시 Datapath (연산 수행, 레지스터 포함)와 [cite: 268, 328] [cite_start]Control Unit (명령어 해석 및 제어 신호 생성)으로 나뉜다. [cite: 269, 335]
![PDF p.17: Detailed anatomy of a von Neumann computer]
* [cite_start]프로그램은 메모리에 저장된 명령어의 집합이다. [cite: 215]
* [cite_start]프로세서는 **Program Counter (PC)**가 가리키는 메모리 주소에서 명령어를 가져와(Fetch) [cite: 207, 209] [cite_start]해독하고(Decode), [cite: 210] [cite_start]입력을 읽어(Read Inputs), [cite: 211] [cite_start]실행하며(Execute), [cite: 212] [cite_start]결과를 쓰고(Write Output), [cite: 213] [cite_start]다음 명령어를 가리킨다(Next Instr). [cite: 214]
![PDF p.13: Instruction execution cycle flowchart]

#### 1.4. ISA의 요소
* [cite_start]**Operations:** 산술/논리 연산, 메모리 접근, 제어 흐름(분기) 등. [cite: 358]
* [cite_start]**Operands:** 데이터의 타입 (레지스터, 상수, 메모리 주소) [cite: 360] [cite_start]및 크기 (byte, 32-bit, 64-bit). [cite: 361]
* [cite_start]**Memory Space:** 주소 지정 방식 및 프로그램이 사용할 수 있는 메모리 크기. [cite: 363, 364]
...
#### 2.1. ISA 설계 목표
1.  **Programmability:** 프로그램을 효율적으로 표현하기 쉬워야 한다. [cite_start]과거에는 사람(어셈블리 프로그래머)이 대상이었으나 [cite: 960, 961] [cite_start]현재는 컴파일러가 대상이다. [cite: 964, 965]
2.  [cite_start]**Implementability (Performance):** 고성능 (저전력, 저비용) 구현이 용이해야 한다. [cite: 947-950] [cite_start]가변 길이 명령어 등은 디코딩을 복잡하게 하여 구현을 어렵게 한다. [cite: 977]
3.  [cite_start]**Compatibility:** 기술이 발전해도 기존 프로그램을 계속 지원(Backward)하거나 [cite: 1067] [cite_start]오래된 프로세서가 새 프로그램을 지원(Forward)할 수 있어야 한다. [cite: 1069]

#### 2.2. 성능 분석
컴퓨터 프로그램의 실행 시간은 세 가지 요소로 결정된다:
* [cite_start]**Execution time = (Instructions / Program) \* (Seconds / Cycle) \* (Cycles / Instruction)** [cite: 997, 1001]
    * `Instructions / Program`: 명령어 수. (ISA, 컴파일러에 영향) [cite_start][cite: 1004]
    * `Seconds / Cycle`: 클럭 속도. (Microarchitecture, 기술에 영향) [cite_start][cite: 1011]
    * `Cycles / Instruction` (CPI): 명령어 당 사이클 수. (ISA, Microarchitecture에 영향) [cite_start][cite: 1007]

#### 2.3. CISC (Complex Instruction Set Computer)
* [cite_start]**특징:** 복잡하고 강력한 명령어들을 제공한다. [cite: 1019] (예: x86, VAX) [cite_start][cite: 1413]
* [cite_start]**장점:** `Instructions/Program` (명령어 수)를 줄이는 것을 목표로 한다. [cite: 1020, 1469] [cite_start]코드 밀도가 좋다. [cite: 1470]
* [cite_start]**단점:** `Cycles/Insn` (CPI) 또는 `Seconds/Cycle` (클럭 주기)가 증가하는 경향이 있다. [cite: 1021, 1469] [cite_start]디코딩 로직이 복잡하다 (특히 x86은 1~16바이트의 가변 길이 명령어). [cite: 1105, 1124, 1125]
* [cite_start]**현황 (x86):** 호환성(Inertia) 덕분에 데스크톱 시장을 장악했다. [cite: 1539-1541]

#### 2.4. RISC (Reduced Instruction Set Computer)
* **특징:** 간단하고 균일한 명령어 집합을 제공한다. (예: MIPS, ARM, RISC-V) [cite_start][cite: 1410]
* [cite_start]**RISC 설계 원칙:** [cite: 1449-1463]
    1.  [cite_start]**Load/Store Architecture:** 메모리 접근은 `load`, `store` 명령어로만 수행한다. [cite: 1454] ALU 연산은 레지스터 간에만 수행한다.
    2.  [cite_start]**Fixed-length Instruction:** 모든 명령어가 고정된 길이(예: 32-bit)를 가져 디코딩이 단순하다. [cite: 1458]
    3.  [cite_start]**Many Registers:** 컴파일러가 최적화에 사용할 수 있는 범용 레지스터가 많다. [cite: 1462]
    4.  [cite_start]**Simple Addressing Modes:** 단순한 주소 지정 방식만 사용한다. [cite: 1456]
* [cite_start]**장점:** `Cycles/Insn` (CPI)과 `Seconds/Cycle` (클럭 주기)를 줄이는 것을 목표로 한다. [cite: 1024, 1472, 1474]
* [cite_start]**단점:** `Instructions/Program` (명령어 수)가 CISC보다 늘어날 수 있으나, 컴파일러 최적화로 상쇄한다. [cite: 1025]
* [cite_start]**현황 (ARM):** 저전력 특성으로 임베디드 및 모바일 시장을 장악했다. [cite: 1550]

#### 2.5. 현대의 CISC: "RISC Inside"
* [cite_start]현대의 고성능 x86 프로세서(CISC)는 내부적으로 **micro-ops (μops)**라는 RISC 스타일의 단순한 명령어로 변환하여 실행한다. [cite: 1496, 1501]
* [cite_start]복잡한 x86 명령어 (예: `addl %ebx, -4(%rax)`)는 여러 개의 μops (load, add, store)로 분해(cracking)된다. [cite: 1517]
* [cite_start]이를 통해 CISC의 호환성을 유지하면서 RISC의 고성능 구현 기술(파이프라이닝, 비순차 실행)을 활용한다. [cite: 1501, 1502]
...
#### 3.1. RISC-V 개요
* [cite_start]UC Berkeley에서 2010년 시작된 오픈 소스 ISA 표준이다. [cite: 1581, 1770]
* [cite_start]라이선스 비용 없이 누구나 RV 칩을 설계할 수 있다. [cite: 1775]
* [cite_start]Google, Intel, Samsung, NVIDIA 등 많은 기업이 참여하는 **RISE (RISC-V Software Ecosystem)**를 통해 생태계가 확장되고 있다. [cite: 1751-1763]
![PDF p.85: RISE Members logo slide]
* [cite_start]**RV32I**는 47개의 기본 정수 명령어를 가진 32-bit 기본 ISA이다. [cite: 773, 1799]
* [cite_start]모든 명령어는 32-bit 고정 길이이다. [cite: 769]
* [cite_start]32개의 32-bit 범용 레지스터 (`x0` ~ `x31`)를 가지며, `x0`는 항상 0이다. [cite: 770, 772]

#### 3.2. 메모리와 주소 지정
* [cite_start]**Byte-Addressable:** 메모리의 최소 주소 단위는 바이트(8 bit)이다. [cite: 1266, 1291]
* [cite_start]**Endian-ness:** RISC-V는 **Little-endian** 방식을 사용한다. [cite: 1943] (즉, `0x12345678` 저장 시 `0x00`번지에 `0x78`이 저장됨)[cite_start]. [cite: 2014, 2015]
* [cite_start]**Alignment:** RISC-V는 하드웨어에서 **misaligned access** (정렬되지 않은 주소 접근)를 투명하게 처리한다 (단, 속도는 느림). [cite: 1905, 1907]
* [cite_start]**Addressing Modes:** RISC-V는 4가지 기본 주소 지정 방식을 사용한다: [cite: 1231]
    1.  [cite_start]Immediate: 상수 값을 명령어 자체에 포함. [cite: 1232]
    2.  [cite_start]Register: 피연산자가 레지스터에 있음. [cite: 1234]
    3.  [cite_start]Base (Displacement): `lw x10, 12(x13)` (x13 레지스터 값 + 12 바이트 오프셋). [cite: 1237]
    4.  [cite_start]PC-relative: PC 기준 상대 주소 (주로 분기 명령어). [cite: 1240]
![PDF p.60: RISC-V addressing modes diagram]

#### 3.3. 함수 호출 규약 (Function Call Convention)
* [cite_start]**6단계:** 1. 파라미터 전달 -> 2. 제어 전달 -> 3. 로컬 저장소 확보 -> 4. 작업 수행 -> 5. 결과 전달 -> 6. 제어 복귀. [cite: 2093]
* **명령어:**
    * `jal ra, FuncName`: 함수 호출. [cite_start]`PC+4` (복귀 주소)를 `ra` (Return Address, x1) 레지스터에 저장하고 `FuncName`으로 점프한다. [cite: 2112]
    * `jr ra` (pseudo-insn): 함수 복귀. [cite_start]`ra` 레지스터에 저장된 주소로 점프한다 (`jalr x0, ra, 0`와 동일). [cite: 2114]
* **레지스터 규약:**
    * **Arguments (a0-a7 / x10-x17):** 함수에 인자를 전달. (Caller-saved) [cite_start][cite: 2095, 2100]
    * **Return values (a0-a1):** 함수가 결과를 반환. (Caller-saved) [cite_start][cite: 2095]
    * **Caller-saved (t0-t6):** 호출한 함수(Caller)가 필요시 저장해야 한다. [cite_start]피호출 함수(Callee)는 마음대로 덮어쓸 수 있다. [cite: 2096, 2099, 2132]
    * [cite_start]**Callee-saved (s0-s11):** 피호출 함수(Callee)가 사용하려면 반드시 저장했다가 복귀 전에 복원해야 한다. [cite: 2096, 2099, 2132]
* **The Stack (스택):**
    * [cite_start]레지스터에 담을 수 없는 로컬 변수, `ra` (중첩 호출 시), `callee-saved` 레지스터를 임시 저장하는 메모리 공간이다. [cite: 2027, 2118, 2208]
    * [cite_start]`sp` (Stack Pointer, x2) 레지스터가 스택의 최상단(가장 마지막에 사용된 주소)을 가리킨다. [cite: 2118]
    * RISC-V에서 스택은 높은 주소에서 낮은 주소로 "자란다". (Push = `sp` 감소, Pop = `sp` 증가) [cite_start][cite: 2118, 2140]
![PDF p.213: RV32 Memory Allocation diagram]

#### 3.4. RISC-V 6가지 명령어 형식 (Instruction Formats)
![PDF p.226: The 6 RISC-V instruction formats (R, I, S, B, U, J)]
* **R-Format (Register):** 레지스터 간 연산. (예: `add`, `sub`, `sll`)
    * [cite_start]`funct7 | rs2 | rs1 | funct3 | rd | opcode` [cite: 2170]
* **I-Format (Immediate):** 상수 값(Immediate)을 사용한 연산 및 로드. (예: `addi`, `lw`, `jalr`)
    * [cite_start]`imm[11:0] | rs1 | funct3 | rd | opcode` [cite: 2179]
* **S-Format (Store):** 메모리 저장. (예: `sw`, `sb`)
    * [cite_start]`imm[11:5] | rs2 | rs1 | funct3 | imm[4:0] | opcode` [cite: 2193]
* **SB-Format (Branch):** 조건부 분기. (예: `beq`, `bne`)
    * [cite_start]`imm[12|10:5] | rs2 | rs1 | funct3 | imm[4:1|11] | opcode` [cite: 2207]
    * PC-relative 주소 사용. Immediate 값들이 흩어져 인코딩된다.
* **U-Format (Upper Immediate):** 20-bit 상위 상수 로드. (예: `lui`)
    * [cite_start]`imm[31:12] | rd | opcode` [cite: 2221]
    * 주로 `addi`와 조합하여 32-bit 상수를 만든다. (예: `lui x10, 0x87654` -> `addi x10, x10, 0x321`) [cite_start][cite: 2224]
* **UJ-Format (Jump):** 무조건 점프. (예: `jal`)
    * [cite_start]`imm[20|10:1|11|19:12] | rd | opcode` [cite: 2231]
    * PC-relative 주소 사용. Immediate 값들이 흩어져 인코딩된다.

#### 3.5. 프로그램 빌드 과정
1.  [cite_start]**Compiler:** C 코드 (`.c`)를 어셈블리 코드 (`.s`)로 변환한다. [cite: 2282]
2.  [cite_start]**Assembler:** 어셈블리 코드 (`.s`)를 기계어 코드인 오브젝트 파일 (`.o`)로 변환한다. [cite: 2283]
    * [cite_start]Pseudo-instructions (예: `j label`)을 실제 명령어 (예: `jal x0, label`)로 변환한다. [cite: 2282]
    * [cite_start]**Symbol Table** (다른 파일이 참조할 수 있는 이 파일의 레이블/데이터 목록)과 [cite: 2293]
    * [cite_start]**Relocation Table** (이 파일이 참조하는 외부 주소 목록)을 생성한다. [cite: 2295]
![PDF p.180: Assembler and Linker process diagram]
3.  [cite_start]**Linker:** 여러 오브젝트 파일 (`.o`)과 라이브러리를 묶어 하나의 실행 파일 (`a.out`)로 만든다. [cite: 2298]
    * [cite_start]Relocation Table을 참조하여 외부 레이블이나 데이터의 절대 주소를 찾아 기계어 코드를 완성(수정)한다. [cite: 2301, 2302]