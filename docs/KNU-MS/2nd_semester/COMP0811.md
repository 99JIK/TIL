...
### 1. 컴퓨터 아키텍처 개요 (p. 5-10)

* 컴퓨터는 유한 상태 기계(Finite State Machine)로 모델링된다 (p. 5).
* 주요 구성 요소는 레지스터(Registers), 메모리(Memory), 프로그램 카운터(PC)이다 (p. 5).
* 기본 실행 모델은 순차적(Sequential Model)이며, Fetch, Decode, Read Inputs, Execute, Write Output, Next Instr 단계를 반복한다 (p. 5-6).
* 이는 폰 노이만 아키텍처(von Neumann architecture)로 알려져 있다 (p. 7).
* 핵심 아이디어는 Stored-Program Computer로, 메모리에 명령어(Instructions)와 데이터(Data)를 함께 저장한다 (p. 8).
* ![PDF p. 9: 폰 노이만 아키텍처 다이어그램 (Input, Control Unit, ALU, Memory, Output)]


## 2. 컴퓨터 아키텍처의 8가지 위대한 아이디어 (p. 12)

* 강의에서는 다음 8가지 핵심 아이디어를 소개한다 (p. 12).
...
## 3. 아이디어 1: Moore's Law (p. 14-29)

* **정의**: Intel 공동 창립자 Gordon Moore가 1965년에 제시한 법칙으로, 칩의 트랜지스터 수가 약 24개월마다 2배씩 증가한다는 것이다 (p. 14).
* 이는 DRAM 밀도(DRAM density)에 해당하며, 대중적으로는 성능이 18개월마다 2배가 된다고 해석되었으나 이는 정확하지 않다 (p. 14).
* ![PDF p. 16: 1970-2020년 트랜지스터 개수 증가 그래프]
* ![PDF p. 17: 트랜지스터 밀도(transistors/mm^2) 그래프. "Moore's Law is still alive!"]
* **성능의 정체**:
    * Moore's Law에 따라 트랜지스터 수는 계속 증가했지만, CPU 성능(GFLOPS) 증가는 10년간 4.15배에 그쳐, 트랜지스터 증가율을 따라가지 못했다 (p. 18).
    * 특히 CPU 클럭 속도(CPU Clock Speed)는 2005년경 "CPU Speed Flat" 상태가 되었다 (p. 19).
    * ![PDF p. 23: 50년간 마이크로프로세서 트렌드 (Transistors, Single-Thread Performance, Frequency, Power, Logical Cores)]
* **AI/ML 요구와의 불일치**: AI 모델의 연산 요구량(GFLOPS)은 4개월마다 2배씩 증가(10년간 43,000,000배)했지만, CPU(4.1배) 및 GPU(21배)의 성능 향상은 이를 따라가지 못하여 큰 격차(gap)가 발생했다 (p. 20).
    * ![PDF p. 20: AI 모델 수요와 CPU/GPU 성능 격차 그래프]
* **Dennard Scaling의 붕괴**:
    * Dennard Scaling은 트랜지스터가 작아져도 전력 밀도(power density)는 일정하게 유지된다는 이론이다 (p. 25).
    * $Power = Capactive Load \times Voltage^2 \times Frequency$ (p. 25).
    * 이 법칙 덕분에 과거에는 트랜지스터를 2배로 늘려도 전력 소모가 일정했다 (p. 25).
    * 하지만 2005-2006년경, 트랜지스터가 너무 작아져 전류 누수(current leakage) 문제로 Dennard Scaling이 붕괴되었다 (p. 26).
* **Power Wall**:
    * 전압을 더 낮출 수 없고, 발생하는 열을 더 제거할 수 없는 '전력 장벽(Power wall)'에 도달했다 (p. 27).
    * ![PDF p. 27: 클럭 속도와 전력 소모 트렌드 (2004년경 정체)]
    * 이에 대한 해결책으로 단일 코어의 클럭을 높이는 대신, 여러 개의 코어(Multi-cores)를 사용하는 방향으로 전환되었다 (p. 28).
* **기타 장벽**: Power wall 외에도 명령어 수준 병렬성(ILP wall) 및 메모리 지연 시간(Memory wall) 문제도 성능 향상을 가로막았다 (p. 29).


## 4. 아이디어 2 & 3: 추상화 및 Amdahl's Law (p. 37, 40)

* **추상화(Abstraction)**: 복잡성을 다루기 위해 하위 레벨의 세부 사항을 숨기는 방식이다. (예: Application -> Algorithm -> ISA -> Physics) (p. 37).
* **Amdahl's Law (Make Common Case Fast)**:
    * 최적화의 이론적 상한선은 코드의 순차적(serial) 부분(병렬화 불가능한 부분)에 의해 제한된다 (p. 40).
    * $f$ = 최적화 가능한 부분의 비율, $s$ = 해당 부분의 속도 향상 배율
    * $Speedup_{enhanced} = \frac{1}{(1 - f) + f/s}$ (p. 40, 136).
    * ![PDF p. 40: Amdahl's Law 실행 시간 비교 (baseline vs enhanced)]


## 5. 아이디어 4, 5, 6: 병렬성, 파이프라이닝, 예측 (p. 42-48)

* **병렬성(Parallelism)**:
    * 애플리케이션에 병렬성이 많다면, 다수의 단순한 코어(GPU 방식)가 소수의 복잡한 코어(CPU 방식)보다 효율적이다 (p. 42).
    * 종류 (p. 43):
...
* **파이프라이닝(Pipelining)**: 작업을 여러 단계로 나누어 동시에 처리한다 (p. 44).
    * ![PDF p. 44: 파이프라인 작업 비유 (토끼 공장)]
* **예측(Prediction)**:
    * Branch Prediction: `if` 문과 같은 분기문에서 다음에 실행될 명령어를 예측하여 성능 저하를 막는다 (p. 46).
    * Prefetching: 향후 접근할 메모리 주소를 예측하여 데이터를 미리 캐시(Cache)로 가져온다 (p. 47-48).


## 6. 아이디어 7: 메모리 계층 (p. 50-54)

* **The Memory Wall Problem**: 프로세서 속도와 메모리 대역폭(memory bandwidth) 간의 성능 격차가 시스템 성능을 제한한다 (p. 50).
* ![PDF p. 50: 메모리 월 문제 (모델 파라미터 vs 메모리 용량)]
* Jim Gray는 "RAM Locality is King"이라고 강조했다 (p. 52).
* **메모리 계층(Memory Hierarchy)**: 속도가 빠르고 용량이 작은 저장장치부터 느리고 용량이 큰 저장장치까지 계층적으로 구성한다 (p. 53).
    * Registers (1 cycle) -> Caches (~10 cycles) -> Main memory (~100 cycles) -> SSD (~1M cycles) -> HDD (~10M cycles).
* ![PDF p. 54: 메모리 계층 피라미드]


## 7. 아이디어 8: 중복성 (p. 56-59)

* **신뢰성(Dependability)**: 시스템 일부가 실패하더라도 전체 시스템이 중단되지 않도록 중복성을 사용한다 (p. 56).
* 예: 3개의 모듈 중 2개가 동의하는 방식 (p. 57), Redundant Arrays of Independent Disks (RAID) (p. 58), Error Correcting Code (ECC) Memory (p. 58).
* SpaceX의 Crew Dragon 모듈은 방사선으로 인한 오류 검사를 위해 3대의 컴퓨터를 사용한다 (p. 59).
* ![PDF p. 57: 3-모듈 중복성 (1+1=1 실패)]


## 8. 성능 측정 지표 (p. 79-88)

* **Latency (지연 시간)**: 작업을 완료하는 데 걸리는 시간. 낮을수록 좋다 (p. 79).
* **Throughput (처리량)**: 단위 시간당 완료하는 작업의 양 (예: Bandwidth). 높을수록 좋다 (p. 79).
* Latency와 Throughput은 종종 상충 관계(contradictory)이다 (p. 81).
* 예: 10TB 데이터 전송 시, 인터넷(low latency) vs. AWS Snowball (high throughput) (p. 81-82).
* 예: 자동차(low latency) vs. 버스(high throughput) (p. 85).
* 기타 지표: Cost (비용), Power (전력, $Joules/sec$), Energy (에너지, $Joules$), Reliability (신뢰성, $MTTF$) (p. 79-80).


## 9. CPU 성능 방정식 (p. 90-98)

* 성능을 결정하는 가장 기본적인 방정식은 다음과 같다 (p. 90, 95).
* $Execution Time = \frac{Instructions}{Program} \times \frac{Cycles}{Instruction} \times \frac{Seconds}{Cycle}$
* 즉, **$ET = IC \times CPI \times CT$** (p. 95).
* **IC (Instruction Count)**: 프로그램의 동적 명령어 수. (Compiler, ISA에 영향) (p. 96).
* **CPI (Cycles Per Instruction)**: 명령어당 평균 사이클 수. (Micro-architecture에 영향) (p. 96).
    * 명령어 종류(ALU, Load, Store, Branch)마다 CPI가 다르다 (p. 98).
* **CT (Cycle Time)**: 1 사이클에 걸리는 시간. ($1 / Frequency$) (p. 96).
* **IPC (Instructions Per Cycle)**: $1 / CPI$. CPI보다 자주 쓰이며 높을수록 좋다 (p. 97).
* **MIPS**: Millions of Instructions Per Second. 성능을 오해하게 만들 수 있는 지표이다 (p. 101).


## 10. 벤치마킹 및 성능 법칙 (p. 119-158)

* **벤치마크 스위트(Benchmark Suites)**: 공정한 비교를 위해 동일한 프로그램과 입력으로 시스템을 평가하는 표준화된 워크로드이다. (예: SPEC, MLPerf) (p. 119).
* **평균 계산 (p. 123)**:
    * Latency (시간)의 평균: Arithmetic Mean (산술 평균).
    * Throughput (비율)의 평균: Harmonic Mean (조화 평균).
    * Speedup (비율)의 평균: Geometric Mean (기하 평균).
* **Gustafson's Law (Weak Scaling)**: Amdahl's Law와 달리, 프로세서 수($P$)가 증가할 때 노드당 작업량을 고정한다. 즉, 전체 문제의 크기가 $P$에 비례하여 커진다. (p. 74).
* **Little's Law**: 큐(Queue) 시스템에 적용되는 법칙. $L = \lambda W$ (p. 145).
    * $L$ = 시스템 내 아이템 수 (대기열 길이)
    * $\lambda$ = 평균 도착률 (Arrival Rate)
    * $W$ = 평균 대기 시간 (Wait Time)
* **M/M/1 Queue**: 대기열 모델. 시스템 처리량(Throughput)이 높아지면(서버가 바빠지면) 대기열(L)이 기하급수적으로 길어져 Latency(W)가 급증한다 (p. 150).
    * ![PDF p. 150: M/M/1 큐 Latency vs Throughput 트레이드오프]
* **Operational Intensity (Arithmetic Intensity)**: $Flops / byte$. 메모리 1바이트당 수행하는 연산 횟수 (p. 154).
* **Roofline Model**: Operational Intensity(X축)와 성능(FLOP/s, Y축) 간의 관계를 시각화한다 (p. 156-157).
    * ![PDF p. 157: Roofline Model (Memory Bound vs Compute Bound)]
