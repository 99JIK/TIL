...
[cite_start]주요 제어 신호(Control Signals)는 다음과 같이 정의된다[cite: 695].
* [cite_start]PCSel: 다음 PC 값을 PC+4로 할지 분기 목표 주소로 할지 결정한다[cite: 697].
* [cite_start]RegWE: 레지스터 파일에 값을 쓸지 여부를 결정한다[cite: 699].
* [cite_start]BrUn: 비교 연산 시 부호 없는 정수로 처리할지 결정한다[cite: 700].
* [cite_start]ASel/BSel: ALU의 입력으로 레지스터 값을 사용할지, PC나 Immediate 값을 사용할지 선택한다[cite: 701].
* [cite_start]MemRW: 메모리 읽기/쓰기를 제어한다[cite: 702].
* [cite_start]WBSel: 레지스터에 기록할 데이터를 메모리, ALU 결과, PC+4 중 무엇으로 할지 선택한다[cite: 705].

![PDF p.629: Comprehensive Diagram of Single-Cycle Datapath with Control Logic Signals]

### Performance and Tradeoffs

[cite_start]CPU 설계에는 성능, 비용, 전력 소모 간의 상충 관계(Tradeoff)가 존재한다[cite: 1231].
성능에 관한 Iron Law는 다음과 같다:
[cite_start]$\frac{Time}{Program} = \frac{Instructions}{Program} \cdot \frac{Cycles}{Instruction} \cdot \frac{Time}{Cycle}$[cite: 1243].

Single-Cycle Datapath 설계의 특징은 다음과 같다.
* [cite_start]CPI (Cycles Per Instruction)는 1이다[cite: 1256].
* [cite_start]클럭 주기($t_{CK}$)는 가장 느린 명령어(주로 Load 명령어)의 경로에 맞춰야 하므로 길어진다[cite: 1262].
* [cite_start]빠른 명령어도 느린 클럭 주기에 맞춰 실행되므로 비효율적이고 유연성이 떨어진다[cite: 1263, 1264].

[cite_start]이러한 단점을 극복하고 클럭 주기를 줄이기 위해 Pipelining 기법이 사용된다[cite: 1321]. [cite_start]Pipelining은 Datapath를 여러 단계로 나누어 동시에 여러 명령어를 처리함으로써 전체적인 처리량(Throughput)을 높인다[cite: 1323].

### Other Architectures

강의에서는 RISC-V 외에도 다른 ISA의 구현을 간략히 다룬다.
* [cite_start]ARMv4: Single-cycle 및 Multi-cycle 구현이 존재하며, PC가 R15 레지스터로 취급되는 등의 차이가 있다[cite: 968, 1031, 1093].
* [cite_start]MIPS: RISC-V와 매우 유사한 구조를 가지며 Single-cycle 구현 예시가 제시되었다[cite: 1172].
## L5