...
일반적으로 루프를 조기에 탈출하거나 `switch`의 나머지 부분을 건너뛸 때 사용한다.
...
![PDF p.39: Sequence and Selection Activity Diagrams](img/Pasted%20image%2020251230173701.png)
![PDF p.40: Repetition Activity Diagrams](img/Pasted%20image%2020251230173714.png)
...
`JPanel`은 `paintComponent` 메서드를 가지며, 시스템은 `JPanel`을 표시해야 할 때마다 이 메서드를 호출한다.
...
`DrawPanel`을 화면에 표시하기 위해 `JFrame` 객체를 생성하여 창을 만든다.
`JFrame`의 `setDefaultCloseOperation` 메서드는 창을 닫을 때 애플리케이션을 종료하도록 설정한다.
`add` 메서드는 `DrawPanel`을 `JFrame`에 부착하고, `setSize`는 창의 크기를 설정하며, `setVisible(true)`는 창을 화면에 표시한다.
```java
// DrawPanel.java
