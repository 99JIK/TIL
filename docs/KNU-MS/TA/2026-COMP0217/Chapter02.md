...
## 2. 첫 번째 Java 프로그램
### 2.1 Java 애플리케이션의 구조
Java 애플리케이션은 java 명령어를 사용하여 JVM(Java Virtual Machine)을 실행할 때 수행되는 컴퓨터 프로그램이다. 모든 Java 프로그램은 적어도 하나의 클래스 정의를 포함해야 한다. `class` 키워드는 클래스 선언을 시작하며 바로 뒤에 클래스 이름이 온다. 클래스 이름은 대문자로 시작하고 각 단어의 첫 글자를 대문자로 표기하는 것이 관례이다. Java는 대소문자를 구별하므로 `a1`과 `A1`은 서로 다른 식별자이다.
...
## 3. 프로그램 수정 및 출력 형식화
### 3.1 print와 println
`System.out.print`는 문자열을 출력하고 커서를 다음 줄로 이동시키지 않는다. 따라서 다음에 출력되는 문자는 print가 출력한 마지막 문자 바로 뒤에 나타난다. 반면 `println`은 출력 후 줄 바꿈 문자를 출력한다.

### 3.2 이스케이프 시퀀스
백슬래시(`\`) 문자를 사용하여 이스케이프 시퀀스를 나타낼 수 있다. `\n`은 줄 바꿈, `\t`는 탭, `\r`은 캐리지 리턴, `\\`는 백슬래시 출력, `\"`는 큰따옴표 출력을 의미한다.
...
### 3.3 printf 메소드
`System.out.printf` 메소드는 데이터를 형식화하여 출력한다. 첫 번째 인자는 형식 문자열(format string)이며 고정 텍스트와 형식 지정자(format specifier)를 포함할 수 있다. 형식 지정자는 `%` 기호로 시작하며 데이터 타입을 나타내는 문자가 뒤따른다. `%s`는 문자열을 위한 자리 표시자이다.
...
## 6. 산술 연산
### 6.1 산술 연산자
Java의 산술 연산자에는 덧셈(`+`), 뺄셈(`-`), 곱셈(`*`), 나눗셈(`/`), 나머지(`%`)가 있다. 정수 나눗셈은 정수 몫을 반환하며 소수점 이하의 부분은 버려진다. 나머지 연산자는 나눗셈 후의 나머지를 반환한다.

### 6.2 연산자 우선순위
곱셈, 나눗셈, 나머지 연산은 덧셈과 뺄셈보다 우선순위가 높다. 같은 우선순위를 가진 연산자들은 일반적으로 왼쪽에서 오른쪽으로 계산된다(결합 법칙). 대입 연산자(`=`)는 가장 낮은 우선순위를 가지며 오른쪽에서 왼쪽으로 결합한다.

![PDF p.25: Step-by-step evaluation of a polynomial expression showing precedence](img/Pasted%20image%2020251230153201.png)

## 7. 의사 결정: 동등 및 관계 연산자
### 7.1 if 문과 조건
`if` 문은 조건의 값에 따라 결정을 내릴 수 있게 한다. 조건은 참(true) 또는 거짓(false)이 될 수 있는 표현식이다. `if` 문은 키워드 `if`로 시작하며 괄호 안에 조건이 온다.
