---
title: Apache Kafka
description: Kafka 아키텍처·용어·토픽/파티션 구조·프로듀서/컨슈머 개념 정리.
tags: [kafka, streaming, distributed-system, message-queue]
sidebar_position: 2
last_update:
  date: 2026-04-24
---
...
Kafka 아키텍처·용어·토픽/파티션 구조·프로듀서/컨슈머 개념 정리.

## Kafka Architecture

![카프카 구성도](./img/Kafka Architecture.png)
...
| Event(Record, Message) | Kafka에서 사용하는 데이터 단위이다. 구성 요소는 Topic, Partition, Key, Value, Header이다. 이 중 필수 값은 Topic과 Value이다. 양식은 보통 JSON, Avro, Protobuffer 등을 사용한다. 해당 데이터는 직렬화를 통해 Byte Array로 변환해 전송한다. |
...
| Partition              | Topic은 Partition으로 구성되어 있다. Partition 마다 Offset이 따로 관리된다. Partition 내에서는 순서가 보장된다. Event가 어떤 Partition에 저장되는 지는 보통 Key 값에 의해 결정된다. 같은 Key 값을 사용할 경우 같은 Partition에 저장된다. |
| Segment                | Producer가 전송한 Event의 실제 물리 파일이다. Partition 마다 Segment가 분리되어 있다.                                                                                                                                            |
| ZooKeeper              | 분산 시스템을 구성하기 위해 필요한 분산 합의와 데이터 복사 등의 과정을 효율적으로 처리하기 위해 사용되는 분산 조정 시스템이다. Kafka의 Leader Node를 정하거나 Multi-Node Kafka Cluster에서 Disk 관리 등을 한다.                                                                |
| Schema Registry        | Confluent Kafka에서 개발한 Web Application이다. Kafka Client 사이에서 Event의 Schema를 저장하고 관리한다.                                                                                                                      |
...
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| --bootstrap-server | Topic을 생성할 Kafka Broker의 Server 주소와 Port 번호이다. `--bootstrap-server <server>:<port>`                                                                                                                           |
| --create           | `--topic`: 해당 Topic 명으로 신규 Topic을 생성한다. `--partitions`: Topic의 Partition의 개수이다. `--replication-factor`: Replication의 개수이다(Multi-Node Kafka Cluster). `--bootstrap-server <server>:<port> --create --topic <topic> --partitions <number> --replication-factor <number>` |
| --list             | Broker에 있는 Topic 리스트를 확인한다. `--bootstrap-server <server>:<port> --list`                                                                                                                                      |
| --describe         | `--topic`: 기술된 Topic 명으로 상세 정보를 표시한다. `--bootstrap-server <server>:<port> --describe --topic <topic>`                                                                                                         |
| --delete           | `--topic`: 기술된 Topic을 삭제한다. `--bootstrap-server <server>:<port> --delete --topic <topic>`                                                                                                                     |
...
![Key가 없는 Event 예시](./img/Nonkey.png)
...
| Config                           | Description                                                                                                                                                                                                                                                                                                   |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Broker & Topic Level Config      | Kafka Server에 설정되는 Config이다. Topic의 Config 값은 Broker Level에서 지정한 Config를 기본으로 설정한다. 별도의 Topic Level Config를 설정할 경우 별도 Config를 따른다. 보통 server.properties에 있는 Config는 변경 시 Broker 재기동이 필요한 Static Config이다. Dynamic Config는 kafka-configs를 이용하여 동적으로 Config 변경이 가능하다. |
| Producer & Consumer Level Config | Kafka Client에서 설정되는 Config이다. Client Level에서 설정되므로 kafka-configs로 수정할 수 없으며 Client 수행 시마다 설정 가능하다.                                                                                                                                                                                                |

### Kafka Config의 사용 방법 <a href="#id-1.kafka-kafkaconfig" id="id-1.kafka-kafkaconfig"></a>

| Assortment     | Usage                                                                                                                                                       |
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Config 값 확인    | `kafka-configs --bootstrap-server <host>:<port> --entity-type [brokers \| topics] --entity-name [<broker_id> \| <topic>] --all --describe`                  |
| Config 값 설정    | `kafka-configs --bootstrap-server <host>:<port> --entity-type [brokers \| topics] --entity-name [<broker_id> \| <topic>] --alter --add-config <property>=<value>` |
| Config 값 설정 해제 | `kafka-configs --bootstrap-server <host>:<port> --entity-type [brokers \| topics] --entity-name [<broker_id> \| <topic>] --alter --delete-config <property>` |
