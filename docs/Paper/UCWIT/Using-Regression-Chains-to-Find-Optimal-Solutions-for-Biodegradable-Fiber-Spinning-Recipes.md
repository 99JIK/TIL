...
- 왜 역설계 모델이 다중 목표여야 하는지
	- 물성 간 Trade-off
		- 섬유의 주요 물성은 독립적이지 않으며, 대부분 서로 상충 관계이다.
			- 강도 & 생분해성: 인장강도를 높이기 위해 분자 구조를 더 촘촘하고 안정적인 결정형으로 만들어야 한다. 하지만 이는 생분해성을 저하시킨다.
			- 강도 & 신도: 강도를 높이게 될 경우 신도가 저하하고, 신도를 높이게 될 경우 강도가 저하되게 되는데, 어느 정도의 강도와 신도를 보장하는 섬유를 생산할 때에 이전의 단일 목표로는 한계가 있음