...
Set $$A=\{1,2,3,4\}$$라고 할 때 Relation $$R=\{(a,b)\mid a\ divides\ b\}$$의 Ordered pair
...
{/* 이미지 누락: <img src="./img/file.excalidraw (9).svg" alt="Example's Directed graph 1" class="gitbook-drawing"> */}
...
{/* 이미지 누락: <img src="./img/file.excalidraw (9).svg" alt="Example's Directed graph 2" class="gitbook-drawing"> */}
...
즉 $$\Delta=\{(a,a)\mid a\in A\}$$를 만족할 때 $$R\cup \Delta$$이며 이 Relation은 Diagonal relation이다.
...
$$a,b$$가 Positive Integer일 때 Relation $$R=\{(a,b)\mid a<b\}$$의 Reflexive closure

* Solution\
  $$R$$의 Reflexive closure는 $$R\cup \Delta=\{(a,b)\mid a<b\}\cap\{(a,a)\mid a\in Z\}=\{(a,b)\mid a\leq b\}$$이다.
:::

:::tip
Example

$$a,b$$가 Positive integer일 때 Relation $$R=\{(a,b)\mid a>b\}$$의 Symmetric closure

* Solution\
  R의 Symmetric closure는$$R\cap R^{-1}=\{(a,b)\mid a>b\}\cap \{(b,a)\mid a>b\}=\{(a,b)\mid a\not =b\}$$이다.
:::
...
즉, $$[a]_r=\{s\mid (a,s)\in R\}$$이며 $$b\in[a]_R$$일 경우 $$b$$는 Equivalence class의 Representative라고 하며 어떤 Equivalence class의 Element는 Representative로 사용할 수 있으며 특별한 의미가 없다.
...
$$R$$을 Set $$S$$의 Equivalence relation이라고 할 때 $$R$$의 Equivalence class들은 $$S$$의 Partition을 형성하고, Set $$S$$의 Partition $$\{A_i \mid i\in I\}$$가 주어지면 $$A_i$$를 Equivalence class로 가지는 Equivalence relation $$R$$이 존
:::
...
* Proof\
  $$P(x)$$가 모든 $$x\in S$$에 대해 True가 아니라고 가정할 때 $$P(y)$$가 False인 어떤 $$y\in S$$가 존재하므로 $$A=\{x\in S\mid P(x)\ is\ false\}$$가 Nonempty set이 된다.\
...
Set $$\{1, 2,3,4\}$$에 대한 Partial ordering $$\{(a,b)\mid a\leq b\}$$의 Directed graph
