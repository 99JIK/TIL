...
$$≡ \neg p∧\neg (\neg p∧q)$$ ∵ De Morgan's laws

$$≡ \neg p∧(\neg (\neg p)∨\neg q)$$ ∵ De Morgan's laws

$$≡ \neg p∧(p∨\neg q)$$ ∵ Double negation laws

$$≡ (\neg p∧p)∨(\neg p∧\neg q)$$ ∵ Distributive laws

$$≡ F∨(\neg p∧\neg q)$$ ∵ \neg p∧p=Contingency

$$≡ \neg p∧\neg q$$ ∵ Commutative laws and Identity laws
:::
...
$$\neg ∀xP(x)≡∃x\neg P(x)$$이고 $$\neg ∃xP(x)≡∀x\neg P(x)$$임을 De Morgan's laws for quantifiers라고 부른다.
...
"Some lions do not drink coffee." = $$∃x(P(x)∧\neg R(x))$$\
"Some fierce creatures do not drink coffee." = $$∃x(Q(x)∧\neg R(x))$$
