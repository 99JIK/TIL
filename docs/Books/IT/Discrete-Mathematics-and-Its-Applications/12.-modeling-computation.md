...
즉 $$L(G)=\{w\in T^\ast \mid S \overset{\ast}{\Rightarrow}w\}$$이다.
...
Set $$\{0^n1^n \mid n=0,1,2,\dots\}$$을 생성하는 Phrase-structure grammar
...
$$\{0^m1^n \mid m,n=0,1,2,\dots\}$$의 Phrase-structure grammer를 찾고 어떤 Type의 Language인지 기입
...
{/* 이미지 누락: ![](./img/image (226).png) */}

* Solution\
  {/* 이미지 누락: ![](./img/image (227).png) */}
:::
...
* Solution\
  ...
  즉 $$A^\ast=\{0^n\mid n=0,1,2,\dots\}$$이다.\
...
{/* 이미지 누락: ![](./img/image (228).png) */}

* Solution\
  {/* 이미지 누락: ![](./img/image (229).png) */}
:::
...
* Solution\
  ...
  즉, $$L(M_1)=\{1^n\mid n=0,1,2,\dots\}$$이다.\
  ...
  즉 $$L(M_3)=\{0^n, 0^n10x\mid n=0,1,2,\dots , \text{and }x \text{ is any string}\}$$이다.
:::
...
{/* 이미지 누락: ![](./img/image (23).png) */}

* Solution\
  {/* 이미지 누락: ![](./img/image (25).png) */}
:::
...
{/* 이미지 누락: ![](./img/image (26).png) */}

* Solution\
  {/* 이미지 누락: ![](./img/image (27).png) */}\
  ...
  정리하여 $$L(M)=\{0^n, 0^n01, 0^n11 \mid n\geq 0\}$$이다.
:::
...
$$\{0^n1^n\mid n=0,1,2,\dots\}$$가 Regular가 아님을 증명
...
* Solution\
  ...
  위 Contradiction으로 $$\{0^n1^n\mid n=0,1,2,\dots\}$$는 Regular가 아님을 알 수 있다.\
  {/* 이미지 누락: ![](./img/image (9).png) */}
:::
...
Context-free grammer에 의해 생성된 Language로 표현할 수 없는 Set도 존재하는데 그 중 하나가 $$\{0^n1^n2^n\mid n=0,1,2\}$$이다.\
...
{/* 이미지 누락: ![A representation of a turing machine](./img/image (10).png) */}
...
$$\{0^n1^n\mid n\geq 1\}$$ Set을 Recognize하는 Turing machine
